...
<html lang="en">
	<head>
		...
		<title>DELETE Provided Course</title>
		...
		<div class="container">
			...
			<?php
				...
				if (isset($_POST['delete'])){
					// print_r($_POST);      // Array ( [selected_course] => CSE110-Programming I [delete] => Delete )  
					
					// Getting the course code and course title
					$selectedCourse = $_POST['selected_course']; 
					$courseCode = "";
					$courseTitle = "";
					$flag = false; 
					for ($i = 0; $i < strlen($selectedCourse); $i++){
						if ($selectedCourse[$i] == "-"){
							$flag = true;
							continue;
						}
						if ($flag == false){
							$courseCode .= $selectedCourse[$i]; 
						} else{
							$courseTitle .= $selectedCourse[$i]; 
						}
					}
					// Now my provider id, course code and course title is ready
					// So let's perform the deleting operation. 
					$query = "DELETE FROM courses WHERE CourseCode='$courseCode' AND CourseTitle='$courseTitle' AND ID_CourseProvider='$id'";
					if (mysqli_query($conn, $query)){
						echo "<div class='alert alert-danger' role='alert'>Your ".$courseCode."-".$courseTitle." Course is DELETED</div>";
					} 
				}
			?>
			
			<form action="20_deleteProvidedCourseForTeacher.php" method="post">
				<div class="form-group">
					<h2>Provided Course Deleting Form</h2>
				</div>
				<?php 
				...
				<div class="form-group">
					<div class="input-group mb-3">
					  <label class="input-group-text" for="inputGroupSelect01">Provided Courses</label>
